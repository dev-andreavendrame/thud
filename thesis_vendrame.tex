...
When we create an entity we suppose that the entity that creates the new one has the role with the necessary privileges (for example a service provider or an issuer). Here we will see how a new user (Bob), can be register on the platform in order to start to access the available services.
We assume that Bob has a mobile application that he can access to provide the required information when asked and that talks to the Truthster backend.

At the beggining, the Truthster backend should firstly load its private key, create an AT (Alastria Token), sign it and send it to Bob.
Then Bob loads his private key (locally stored), creates and AIC (Alastria Identity Creation), signs it and sends it to the Truthster backend.
After that Truthster should decode it and verify the signature with the public key of Bob.
When the verification has been done Truthster can send to Alastria blockchain the signed transaction that allows the user to create his AlastriaID. This transaction is a preparation step before creating effectively the AlastriaID.
Finally, after the confirmation of the previous transaction Truthster sends to Alastria the signed (by Bob) transaction to create the AlastriaID. 
The result is that the AlastriaPublicKeyRegistry is updated and now the user public key is stored safely on-chain and the Truthster backend now has all the information neeeded to create a DID that is stored remotely and ready to be used for next scopes.

\textit{Notes}: 
In AlastriaID we can create a new entity and once it is create it has the default role of a Subject. Bob, our new entity, has this specific role and can't issue identities or provide services to other users.
...
\subsection{Add new credentials to Subject}


\subsection{}
AlastriaNameService allows to manage data of entities, the only data that are not hashed on the blockchain. These data are: 
